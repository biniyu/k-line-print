...
// 构造函数。
inline void * operator new(size_t, void *_P, int, int )
        {return (_P); }
#if     _MSC_VER >= 1200
inline void  operator delete(void *, void *, int, int )
	{return; }
#endif

template<class TYPE>
_SPARRAY_INLINE void  SPConstructElements(TYPE* pElements, int nCount)
{
	SP_ASSERT(nCount == 0 ||
...
template<class TYPE>
_SPARRAY_INLINE void  SPDestructElements(TYPE* pElements, int nCount)
{
	SP_ASSERT(nCount == 0 ||
		SP_IsValidAddress((void*)pElements, nCount * sizeof(TYPE)));
...
template<class TYPE>
_SPARRAY_INLINE void  SPCopyElements(TYPE* pDest, const TYPE* pSrc, int nCount)
{
	SP_ASSERT(nCount == 0 ||
...
template<class TYPE>
_SPARRAY_INLINE void  SPSerializeElements(CSPArchive& ar, TYPE* pElements, int nCount)
{
	SP_ASSERT(nCount == 0 ||
		SP_IsValidAddress((void*)pElements, nCount * sizeof(TYPE)));
...
_SPARRAY_INLINE BOOL  SPCompareElements(const TYPE* pElement1, const ARG_TYPE* pElement2)
{
...
struct CSPPlex     // warning variable length structure
{
	...
	static CSPPlex* Create(CSPPlex*& head, UINT nMax, UINT cbElement);
