...
int TryGetReport( BYTE * buffer, size_t len, PRCV_DATA pRCV_DATA )
{
	...
	if( pRCV_DATA && bStockOK && astocks.GetSize() > 0 && datalen >= (int)sizeof(TW_ANS_REPORT) )
	{
		...
		for( int i=0; i<astocks.GetSize() && i<datalen/(int)sizeof(TW_ANS_REPORT); i++ )
		{
			...
			pRCV_DATA->m_pReport[i].m_fAmount	= (float)(preport[i].m_amount);
...
int TryGetHistory( BYTE * buffer, size_t len, PRCV_DATA pRCV_DATA )
{
	...
	size_t dataoffset = 43;
	int datalen = len - dataoffset;
	if( pRCV_DATA && bStockOK && nCount > 0 && datalen >= 1*(int)(sizeof(TW_ANS_HISTORY)+sizeof(DWORD)) )
	{
		int dataprefix = 0;
		if( 1 == nCount && datalen >= nCount*(int)(sizeof(TW_ANS_HISTORY)+sizeof(DWORD))+36 )
			dataprefix = datalen - nCount*(int)(sizeof(TW_ANS_HISTORY)+sizeof(DWORD)) - 36;
		else if( datalen > nCount*(int)(sizeof(TW_ANS_HISTORY)+sizeof(DWORD)) )
			dataprefix = datalen - nCount*(int)(sizeof(TW_ANS_HISTORY)+sizeof(DWORD));
		else
		{
			nCount = datalen / (int)(sizeof(TW_ANS_HISTORY)+sizeof(DWORD));
			dataprefix = datalen - nCount*(int)(sizeof(TW_ANS_HISTORY)+sizeof(DWORD));
		}
